...
// -------- 从这里开始替换原脚本中 service == "YouTube" 的整个块 --------
if (service == "YouTube") {
    ...
    // 翻译并发与批量控制参数（可调）
    const BATCH_SIZE = 5        // 每次并发翻译多少行（越小越接近“实时”，但请求次数更多）
    const CACHE_KEY = "yt_trans_cache_v1" // 缓存键（版本号可改）
    const USER_AGENT = "GoogleTranslate/6.29.59279 (iPhone; iOS 15.4; en; iPhone14,2)"

    // 读取缓存（JSON map: 原文 -> 翻译）
    function loadCache() {
        try {
            let raw = $prefs.valueForKey(CACHE_KEY)
            return raw ? JSON.parse(raw) : {}
        } catch (e) { return {} }
    }
    function saveCache(map) {
        try {
            $prefs.setValueForKey(JSON.stringify(map), CACHE_KEY)
        } catch (e) {}
    }

    // 单行 Google 翻译（返回纯译文字符串）
    async function translateLineGoogle(text) {
        // 空文本直接返回空
        ...
        try {
            ...
            let out = ""
            if (json.sentences && json.sentences.length > 0) {
                for (let s of json.sentences) {
                    if (s.trans) out += s.trans
                }
            } else if (Array.isArray(json)) { // 兼容某些老接口返回
                out = json.map(x => (x && x[0]) ? x[0] : "").join("")
            }
            out = out.replace(/\n/g, " ").trim()
            // 写入缓存（简单策略）
            try { cache[text] = out; saveCache(cache) } catch (e) {}
            return out
        } catch (e) {
            return ""
        }
    ...
    // 并发翻译一批 lines（lines: [{idx,orig,replacePatt}]）
    async function translateBatch(lines) {
        // 并发触发所有请求（Promise.all 限制在 BATCH_SIZE 之内，因为 lines 长度 <= BATCH_SIZE）
        let promises = lines.map(l => translateLineGoogle(l.orig).then(res => ({ idx: l.idx, text: res })))
        let results = await Promise.all(promises)
        return results // [{idx, text}, ...]
    }

    // 主体流程
    $task.fetch(options).then(async response => {
        // 如果用户选 "sl"（仅显示翻译轨），直接把翻译轨返回
        if (setting.line == "sl") $done({ body: response.body })

        // 得到原始 body（原字幕）
        let origBody = body // global body from response
        // 统一去 CR
        origBody = origBody.replace(/\r/g, "")
        // 合并换行避免意外断句
        origBody = origBody.replace(/(\d+:\d\d:\d\d.\d\d\d --> \d+:\d\d:\d\d.\d.+\n.+)\n(.+)/g, "$1 $2")
        origBody = origBody.replace(/(\d+:\d\d:\d\d.\d\d\d --> \d+:\d\d:\d\d.\d.+\n.+)\n(.+)/g, "$1 $2")

        // 提取对话块
        let dialogue = origBody.match(/\d+:\d\d:\d\d.\d\d\d --> \d+:\d\d:\d\d.\d.+\n.+/g)
        if (!dialogue) $done({ body: origBody })
        let timeline = origBody.match(/\d+:\d\d:\d\d.\d\d\d --> \d+:\d\d:\d\d.\d.+/g)

        // For asr kind, remove <s> tags for comparisons
        if (url.match(/&kind=asr/)) {
            origBody = origBody.replace(/<\/?s[^>]*>/g, "")
            response.body = response.body.replace(/<\/?s[^>]*>/g, "")
            dialogue = origBody.match(/\d+:\d\d:\d\d.\d\d\d --> \d+:\d\d:\d\d.\d.+\n.+/g)
            timeline = origBody.match(/\d+:\d\d:\d\d.\d\d\d --> \d+:\d\d:\d\d.\d.+/g)
            if (!dialogue) $done({ body: origBody })
        }
        ...
        for (let i = 0; i < dialogue.length; i++) {
            // 提取该条的纯文本（去掉时间头）
            let text = dialogue[i].replace(/^\d+:\d\d:\d\d.\d\d\d --> \d+:\d\d:\d\d.\d.+\n/, "")
            // 去掉某些标签
            text = text.replace(/<\/*(c\.[^>]+|i|c)>/g, "").trim()
            orig_lines.push({ idx: i, orig: text })
        }

        // 分批并发翻译（小批次）
        for (let start = 0; start < orig_lines.length; start += BATCH_SIZE) {
            let slice = orig_lines.slice(start, start + BATCH_SIZE)
            // 对每条生成替换 regex（保守型，按完整 dialogue 匹配，避免误伤）
            let sliceMap = slice.map(l => ({
                idx: l.idx,
                orig: l.orig,
                dialogueRaw: dialogue[l.idx]
            }))

            // 翻译本批次
            let transResults = await translateBatch(sliceMap) // [{idx, text}, ...]

            // 处理并回写到 origBody（每个翻译成功则立刻回写）
            for (let r of transResults) {
                let j = r.idx
                let translated = r.text || ""
                // 构造匹配 patt：匹配该时间轴整段（保留标签），并插入翻译
                // 根据 setting.line 决定插入位置：s（原行在上，译文下），f（相反）
                let pattRaw = new RegExp(`(${dialogue[j].replace(/([.*+?^=!:${}()\$begin:math:display$$end:math:display$\/\\])/g, "\\$1")})`)
                // 如果 pattern fails (含特殊字符),我们改用时间轴匹配替换
                if (!pattRaw.test(origBody)) {
                    let time = timeline[j].match(/^\d+:\d\d:\d\d/)[0]
                    pattRaw = new RegExp(`${time}.\\d\\d\\d --> \\d+:\\d\\d:\\d\\d.\\d.+(\\n.+)+`)
                }
                ...
                if (translated) {
                    if (setting.line == "s") {
                        // 保持原文在上，译文在下
                        try {
                            origBody = origBody.replace(pattRaw, (m) => `${m}\n${translated}`)
                        } catch (e) {
                            // 回退：简单 append
                            origBody = origBody + `\n${translated}`
                        }
                    } else if (setting.line == "f") {
                        // 译文在上，原文在下
                        try {
                            origBody = origBody.replace(pattRaw, (m) => `${translated}\n${m}`)
                        } catch (e) {
                            origBody = origBody + `\n${translated}`
                        }
                    } else {
            ...
            // 小批次完成后，尽可能保存当前进度到 settings cache（供下次快速命中）
            try {
                settings[service].s_subtitles_url = url
                settings[service].subtitles = origBody
